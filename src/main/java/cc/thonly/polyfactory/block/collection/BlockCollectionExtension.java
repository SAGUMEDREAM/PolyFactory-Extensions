...
public class BlockCollectionExtension extends AbstractElement implements CollisionView {
    public static boolean ignoreCollisions = BlockCollection.ignoreCollisions;
    private final DisplayElement main = ItemDisplayElementUtil.createSimple(ItemStack.EMPTY, 2);
    private final int[] blockId;
    private final ElementHolder[] blockElementHolders;
    ...
    public BlockCollectionExtension(int sizeX, int sizeY, int sizeZ) {
        this(new BlockCollectionData(sizeX, sizeY, sizeZ));
    }

    public BlockCollectionExtension(BlockCollectionData data) {
        var size = data.size();
        this.blockId = new int[size];
        this.blockElementHolders = new ElementHolder[size];
        ...
        for (var x = 0; x < this.data.sizeX(); x++) {
            for (var y = 0; y < this.data.sizeY(); y++) {
                for (var z = 0; z < this.data.sizeZ(); z++) {
                    ...
                    sendInitialBlockWithElementVisual(packetConsumer, x, y, z, pos, vec, quaternion);
    ...
    public void sendInitialBlockWithElementVisual(Consumer<Packet<ClientPlayPacketListener>> packetConsumer, int x, int y, int z, Vec3d pos, Vector3f vec, Quaternionf quaternion) {
        var i = index(x, y, z);
        var elementHolder = this.blockElementHolders[i];
        if (elementHolder != null) {
            IntList entityIds = elementHolder.getEntityIds();
    ...
    public void updateBlockVisualsAt(int x, int y, int z, BlockState state) {
        var i = index(x, y, z);
        BlockWithElementHolder blockWithElementHolder = BlockWithElementHolder.get(state);
        if (blockWithElementHolder != null) {
            var pos = this.main.getLastSyncedPos();
            var vec = new Vector3f();
            var quaternion = new Quaternionf(this.quaternion);
            if (pos != null) {
                ElementHolder holder = blockWithElementHolder.createElementHolder(this.world, new BlockPos((int) pos.x, (int) pos.y, (int) pos.z), state);
                if (holder != null) {
                    this.blockElementHolders[i] = holder;
                    if (this.getHolder() != null) {
                        this.sendInitialBlockWithElementVisual(this.getHolder()::sendPacket, x, y, z, pos, vec, quaternion);
                    }
                }
            }
        }
    ...
    public void setWorld(ServerWorld world) {
        ...
        if (this.world != null) {
            ((BlockCollectionExtensionView) this.world).removeCollision(this);
        }
        if (world != null) {
            ((BlockCollectionExtensionView) world).addCollision(this);
        }
